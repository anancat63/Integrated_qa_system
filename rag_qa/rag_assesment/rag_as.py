...
llm = ChatOllama(model="qwen2.5:7b", base_url='http://localhost:11434')#ollama专用端口
...
"""
{
    ...
    "answer": ...,        # 你RAG系统生成的结果
    "context": ...,       # 你检索器返回的文本
    "ground_truth": ...   # 标准答案（人工或参考答案）
}
这个数据就是RAG系统输出的结果以及标准答案，通过制作这个数据集来评估RAG系统
模型生成两个指标比上下文的两个更重要
此数据是人工标注-->dataset格式-->准备好模型-->使用ragas的评估函数evaluate
