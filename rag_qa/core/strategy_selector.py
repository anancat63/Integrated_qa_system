    ...
    def call_dashscope(self, prompt):
        ...
        try:
            ...
            completion = self.client.chat.completions.create(
                ...
                temperature=0.1#温度值越低的，则生成的结果越接近于输入
            )
...
# 获取策略选择 Prompt模板：Zero-shot text classification（零样本文本分类）
# 输出空间是固定的 4 个标签（直接检索/HyDE/子查询/回溯），模型依据描述直接选标签；没有训练，只靠指令与示例。
...
# 选择策略
    def select_strategy(self, query):
        logger.info(f"开始选择策略，查询为：'{query}'")
        # print("*"*80)
        ...
        # print("*"*80)
        #   调用 LLM 获取检索策略   模板是在模型调用的时候才填充的(用query)
...
if __name__ == '__main__':
    ...
    query = "人工智能在教育行业未来三年的发展趋势是什么？"
    strategy_prompt_template = ss._get_strategy_prompt()
    print(ss.call_dashscope(strategy_prompt_template.format(query=query)))
