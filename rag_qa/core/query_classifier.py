...
class QueryClassifier:
    ...
    def load_model(self):
        # 检查模型路径是否存在
        logger.info(f"检查模型路径ing...: {self.model_path}")
...
#         """
        ...
        # print("TrainingArguments class path:", inspect.getfile(transformers.TrainingArguments))
        # print("TrainingArguments dir:", dir(transformers.TrainingArguments))
        logger.info("设置训练参数TrainingArguments")
        training_args = TrainingArguments(# 训练参数
            ...
            logging_dir="./bert_logs", # 日志保存路径:如果想生成这个文件夹，需要安装tensorboard：只是告诉 Trainer：如果你启用了 TensorBoard 记录，把 event 文件写到哪里。
...
#计算评估指标
    def compute_metrics(self, eval_pred):
        logger.info("计算评估指标ing")
        """计算评估指标"""
        logits, labels = eval_pred
        predictions = np.argmax(logits, axis=-1)
        accuracy = (predictions == labels).mean()#求平均值得到准确率
        return {"accuracy": accuracy}
#评估模型性能
    def evaluate_model(self, texts, labels):
        """评估模型性能"""
        logger.info("评估模型性能ing")
