    ...
    def _retrieve_with_backtracking(self, query, source_filter):
        ...
        try:
            ...
            return self.vector_store.hybrid_search_with_rerank(
                simplified_query,# 使用 template 后缀区分
...
#处理查询
    def generate_answer(self, query, source_filter=None, history=None):#新增历史参数history
        #   记录查询开始时间
        ...
        print(f"构建 Prompt 输入：\n{prompt_input[:500]}...")
        try:
            # 使用大模型获得输出结果：
            logger.info(f"开始调用 LLM (查询: '{query}')并流式输出")
...
if __name__ == '__main__':
    ...
    query = "AI课程的NLP的技术有哪些?"
    answer = rag_system.generate_answer(query= query, source_filter="ai")
    for vlaue in answer:
        print(vlaue)
