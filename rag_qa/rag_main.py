...
def main(query_mode=True, directory_path="data"):
    ...
    if not query_mode:
        ...
        print("\n欢迎使用 RAG 交互式查询系统！")
        print(f"支持的数据源类别：{valid_sources}")
        ...
        while True:
            ...
            source_filter_input = input(f"请输入数据源类别 ({'/'.join(valid_sources)}) (直接回车默认不过滤)：").strip()
            ...
            if source_filter_input:
                if source_filter_input in valid_sources:
                    ...
                    logger.info(f"用户选择了数据源过滤: {source_filter}")
                else:
                    logger.warning(
                        f"无效的数据源类别 '{source_filter_input}'，将不过滤"
                    )
                    print(f"提示：输入的数据源 '{source_filter_input}' 无效，将不过滤。")
...
if __name__ == '__main__':
    # main(query_mode=False, directory_path=r".\data")
    # 或者通过命令行参数控制
    import argparse
